...
public class NullPredicate extends RexNodeBase {
  ...
  private void buildExpr(RexNode node) {
    ...
    if (sqlKind.equals(SqlKind.IS_NULL)) {
      this.output =
          new SymbolicColumn(rexNodeConverter.getOutputNull(), z3Context.mkFalse(), z3Context);
    }
    if (sqlKind.equals(SqlKind.IS_NOT_NULL)) {
      this.output =
          new SymbolicColumn(
              z3Context.mkNot(rexNodeConverter.getOutputNull()), z3Context.mkFalse(), z3Context);
    }
  }
}
