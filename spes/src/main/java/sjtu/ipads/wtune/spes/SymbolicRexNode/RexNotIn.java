...
public class RexNotIn extends RexNodeBase {
  ...
  class SymbolicRexNotIn {
    ...
    public SymbolicRexNotIn(
        SymbolicColumn result, List<SymbolicColumn> symbolicColumns, AlgeNode subquery) {
      this.result = result;
    ...
    public boolean eq(List<SymbolicColumn> symbolicColumns, AlgeNode subquery) {
      boolean result =
          z3Utility.symbolicOutputEqual(
              z3Context.mkTrue(), symbolicColumns, this.symbolicColumns, z3Context);
      if (result) {
  ...
  private void buildExpr(NotIn node) {
    ...
    if (!findMatch) {
      ...
      SymbolicRexNotIn newSymbolicNotIn =
          new SymbolicRexNotIn(this.output, compareColumns, node.getSubQuery());
