...
public class Constant extends RexNodeBase {
  ...
  private void buildExpr(RexNode node) {
    ...
    if (type.equals(SqlTypeName.NULL)) {
      this.output =
          new SymbolicColumn(
              z3Utility.mkDumpValue(constant.getType(), z3Context), z3Context.mkTrue(), z3Context);
    } else {
      this.output =
          new SymbolicColumn(convertRexLiteralNotNull(constant), z3Context.mkFalse(), z3Context);
    }
  }
