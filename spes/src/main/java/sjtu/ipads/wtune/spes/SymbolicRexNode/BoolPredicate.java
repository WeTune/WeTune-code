...
public class BoolPredicate extends RexNodeBase {
  ...
  private void buildNotExpr(RexCall node) {
    ...
    this.output =
        new SymbolicColumn(
            z3Context.mkNot((BoolExpr) converter.getOutputValue()).simplify(),
            converter.getOutputNull(),
            z3Context);
  }

  public static SymbolicColumn getAndNodeSymbolicColumn(
      Collection<RexNode> operands,
      List<SymbolicColumn> inputs,
    ...
    BoolExpr outputNull =
        (BoolExpr)
            z3Context
                .mkAnd(
                    z3Utility.mkAnd(nullValues, z3Context), z3Utility.mkOr(nullSymbol, z3Context))
                .simplify();
    return (new SymbolicColumn(outputValue, outputNull, z3Context));
  }

  public static SymbolicColumn getOrNodeSymbolicColumn(
      Collection<RexNode> operands,
      List<SymbolicColumn> inputs,
      List<BoolExpr> assignConstraints,
      Context z3Context) {
