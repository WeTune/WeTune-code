...
public class AggregateNode extends AlgeNode {
  ...
  public void emptyTableSymbolicColumn() {
    ...
    for (AggregateCall aggregateCall : this.aggregateCallList) {
      if (aggregateCall.getAggregation().getKind().equals(SqlKind.COUNT)) {
        SymbolicColumn value0 =
            new SymbolicColumn(z3Context.mkInt(0), z3Context.mkFalse(), z3Context);
        symbolicColumns.add(value0);
      } else {
        SymbolicColumn nullValue =
            new SymbolicColumn(
  ...
  private boolean groupBySymbolicDecide(
      BoolExpr env, List<SymbolicColumn> groupByColumn1, List<SymbolicColumn> groupByColumn2) {
  ...
  private BoolExpr freshEq(List<SymbolicColumn> groupByColumn) {
    List<SymbolicColumn> freshColumn =
        SymbolicColumn.constructFreshColumns(groupByColumn, z3Context);
    BoolExpr[] columnEqs1 = new BoolExpr[groupByColumn.size()];
    for (int i = 0; i < freshColumn.size(); i++) {
      columnEqs1[i] =
          z3Utility.symbolicColumnEq(groupByColumn.get(i), freshColumn.get(i), z3Context, false);
    }
