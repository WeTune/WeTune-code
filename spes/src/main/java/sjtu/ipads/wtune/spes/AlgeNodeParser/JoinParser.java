...
public class JoinParser extends AlgeNodeParser {
  ...
  private List<AlgeNode> leftJoinAll(
      AlgeNode leftNode, AlgeNode rightNode, RexNode joinCondition, Context z3Context) {
    List<AlgeNode> leftInputs = new ArrayList<>();
    if (leftNode instanceof UnionNode) {
      leftInputs.addAll(leftNode.getInputs());
    } else {
  ...
  private List<AlgeNode> rightJoinAll(
      AlgeNode leftNode, AlgeNode rightNode, RexNode joinCondition, Context z3Context) {
    List<AlgeNode> rightInputs = new ArrayList<>();
    if (rightNode instanceof UnionNode) {
      rightInputs.addAll(rightNode.getInputs());
  ...
  private RexNode inCondition(
      AlgeNode leftNode, AlgeNode rightNode, RexNode joinCondition, boolean isLeft) {
    List<RexNode> leftJoinColumns = new ArrayList<>();
    List<RexNode> rightJoinColumns = new ArrayList<>();
    separateJoinCondition(
        joinCondition, leftJoinColumns, rightJoinColumns, leftNode.getOutputExpr().size());
    if (isLeft) {
      ...
      for (RexNode leftJoinColumn : leftJoinColumns) {
        properLeftJoinColumns.add(
            RexNodeHelper.substitute(leftJoinColumn, leftNode.getOutputExpr()));
      }
      ...
      for (RexNode rightJoinColumn : rightJoinColumns) {
        properRightJoinColumns.add(
            RexNodeHelper.substitute(rightJoinColumn, rightNode.getOutputExpr()));
      }
  ...
  private void separateJoinCondition(
      RexNode joinCondition,
      List<RexNode> leftJoinColumns,
      List<RexNode> rightJoinColumns,
      int offSize) {
    ...
    System.out.println(
        "the SQL kind IS NOT support in separateJoinCondition:" + joinCondition.getKind());
    return;
  }
