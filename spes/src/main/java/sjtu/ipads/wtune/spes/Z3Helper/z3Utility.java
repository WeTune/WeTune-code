package sjtu.ipads.wtune.spes.Z3Helper;
...
public class z3Utility {
  ...
  public static boolean isConditionEq(
      List<BoolExpr> assignConstraints,
      BoolExpr condition1,
      BoolExpr condition2,
      Context z3Context) {
  ...
  public static BoolExpr makeColumnsEq(
      List<SymbolicColumn> columns1, List<SymbolicColumn> columns2, Context z3Context) {
  ...
  public static BoolExpr simpleColumnEq(
      SymbolicColumn column1, SymbolicColumn column2, Context z3Context) {
    BoolExpr valueEqual =
        (BoolExpr)
            z3Context.mkEq(column1.getSymbolicValue(), column2.getSymbolicValue()).simplify();
    BoolExpr isNullEqual =
        (BoolExpr) z3Context.mkEq(column1.getSymbolicNull(), column2.getSymbolicNull()).simplify();
    return (BoolExpr) z3Context.mkAnd(valueEqual, isNullEqual).simplify();
  }

  public static boolean symbolicOutputEqual(
      BoolExpr conditions,
      List<SymbolicColumn> list1,
      List<SymbolicColumn> list2,
      Context z3Context) {
  ...
  public static Expr getDumpVariable(Sort sort, Context z3Context) {
    count++;
    return z3Context.mkConst("dump" + count, sort);
  }

  public static Expr mkDumpValue(RelDataType type, Context z3Context) {
    count++;
    return z3Context.mkConst(
        "dump" + count, RexNodeUtility.getSortBasedOnSqlType(z3Context, type.getSqlTypeName()));
  }

  public static BoolExpr mkBoolPredicate(Context z3Context) {
    count++;
    String name = "B" + count;
  ...
  public static BoolExpr symbolicColumnEq(
      SymbolicColumn column1, SymbolicColumn column2, Context z3Context, boolean checkTrivialEq) {
    ...
    BoolExpr valueEq =
        z3Context.mkAnd(
            z3Context.mkEq(column1.getSymbolicNull(), column2.getSymbolicNull()),
            z3Context.mkEq(column1.getSymbolicValue(), column2.getSymbolicValue()));
    return (BoolExpr) z3Context.mkOr(bothNull, valueEq).simplify();
  }

  private static boolean trivialEqual(SymbolicColumn column1, SymbolicColumn column2) {
  ...
  private static boolean trivialEqual(Expr e1, Expr e2) {
