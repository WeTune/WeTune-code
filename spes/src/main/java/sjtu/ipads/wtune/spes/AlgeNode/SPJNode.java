package sjtu.ipads.wtune.spes.AlgeNode;
...
import sjtu.ipads.wtune.spes.SymbolicRexNode.*;
...
public class SPJNode extends AlgeNode {
  ...
  public SPJNode(
      List<RexNode> outputExpr, Set<RexNode> conditions, List<AlgeNode> inputs, Context z3Context) {
  ...
  public SymbolicColumn constructSymbolicCondition() {
    ...
    if (this.conditions.isEmpty()) {
      this.symbolicCondition =
          new SymbolicColumn(z3Context.mkTrue(), z3Context.mkFalse(), z3Context);
    } else {
      this.symbolicCondition =
          BoolPredicate.getAndNodeSymbolicColumn(
              this.conditions, this.inputSymbolicColumns, assign, z3Context);
    }
  ...
  private boolean checkSymbolicCondition(SPJNode node) {
    ...
    if (z3Utility.isConditionEq(
        env, condition1.isValueTrue(), condition2.isValueTrue(), z3Context)) {
  ...
  public void constructSR() {
    ...
    for (RexNode rexNode : this.outputExpr) {
      RexNodeBase converter =
          RexNodeConverter.rexConstraints(this.inputSymbolicColumns, rexNode, z3Context);
