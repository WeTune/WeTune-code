package sjtu.ipads.wtune.spes.RexNodeHelper;
...
public class RexNodeHelper {
  ...
  private static RexNode unhandled(RexNode node, String procedureName) {
    System.out.println(
        "This rex node is not handled in `" + procedureName + "`: " + node.getClass().toString());
    return node;
  }
  ...
  public static void collectVariables(RexNode input, Set<RexNode> result) {
    if (input instanceof RexInputRef) {
      ...
      ((RexCall) input)
          .getOperands()
          .forEach((RexNode operand) -> collectVariables(operand, result));
    }
  }
}
