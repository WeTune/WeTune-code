...
public class CaseNode extends RexNodeBase {
  ...
  private void buildExpr(RexNode node) {
    ...
    while (count < operands.size() - 1) {
      ...
      RexNodeBase conditionConverter =
          RexNodeConverter.rexConstraints(inputs, condition, z3Context);
  ...
  private BoolExpr getOutEqualValue(RexNode value, BoolExpr condition) {
    ...
    BoolExpr outputEqualValue =
        z3Context.mkEq(this.output.getSymbolicValue(), valueConstrains.getOutputValue());
    BoolExpr nullValue =
        z3Context.mkEq(this.output.getSymbolicNull(), valueConstrains.getOutputNull());
