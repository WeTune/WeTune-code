package sjtu.ipads.wtune.spes.SymbolicRexNode;
...
public class RexNodeConverter {
  public static List<SqlKind> logicSQL =
      Arrays.asList(SqlKind.AND, SqlKind.OR, SqlKind.NOT, SqlKind.IN);
  public static List<SqlKind> isNullSQL = Arrays.asList(SqlKind.IS_NULL, SqlKind.IS_NOT_NULL);
  public static List<SqlKind> arithmeticCompareSQL =
      Arrays.asList(
  ...
  public static List<SqlKind> arithmeticSQL =
      Arrays.asList(SqlKind.PLUS, SqlKind.MINUS, SqlKind.TIMES, SqlKind.DIVIDE);

  public static RexNodeBase rexConstraints(
      List<SymbolicColumn> inputs, RexNode node, Context z3Context) {
  ...
  private static RexNodeBase rexCallConstraints(
      List<SymbolicColumn> inputs, RexNode node, Context z3Context) {
    ...
    if (callNode.isA(SqlKind.CASE)) {
      ...
      System.out.println(
          "`rexCallConstraints` does not handle this type of call node: " + callNode.getClass());
