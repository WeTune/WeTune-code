...
public class UserDeFun extends RexNodeBase {
  public class SymbolicUserDefun {
    ...
    public SymbolicUserDefun(
        String funName, List<SymbolicColumn> args, SymbolicColumn returnValue) {
    ...
    public boolean isEq(String funName, List<SymbolicColumn> args) {
      if (this.funName.equals(funName)) {
        boolean result =
            z3Utility.symbolicOutputEqual(z3Context.mkTrue(), args, this.args, z3Context);
        return result;
      }
      return false;
    }
  ...
  private static Map<String, List<SymbolicUserDefun>> registerFunction = new HashMap<>();
