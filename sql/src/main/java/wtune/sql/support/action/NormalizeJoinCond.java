...
import wtune.sql.util.RenumberListener;

import java.util.List;

import static wtune.sql.SqlSupport.linearizeConjunction;
import static wtune.sql.ast.constants.BinaryOpKind.AND;
...
class NormalizeJoinCond {
  ...
  private static TIntList collectJoinCondition(SqlNode querySpec) {
    ...
    return LocatorSupport.nodeLocator()
        ...
        .stopIf(n -> AND != n.$(ExprFields.Binary_Op))
        .gatherer()
        .gather(whereClause);
  }
  ...
  private static boolean isPlainCondition(SqlNode expr) {
    if (ExprKind.Unary.isInstance(expr))
      return expr.$(ExprFields.Unary_Op) == NOT && isPlainCondition(expr.$(ExprFields.Unary_Expr));
  ...
  private static SqlNode locateJoinSource(SqlNode joinCond) {
    ...
    final List<Relation> inputs = ResolutionSupport.getEnclosingRelation(joinCond).inputs();
