...
import sjtu.ipads.wtune.sql.pg.internal.PGParserBaseVisitor;
...
import static sjtu.ipads.wtune.sql.ast.constants.JoinKind.CROSS_JOIN;
import static sjtu.ipads.wtune.sql.ast.constants.LiteralKind.*;
import static sjtu.ipads.wtune.sql.ast.constants.SetOpKind.*;
import static sjtu.ipads.wtune.sql.ast.constants.UnaryOpKind.NOT;
import static sjtu.ipads.wtune.sql.pg.PgAstHelper.*;
...
public class PgAstBuilder extends PGParserBaseVisitor<SqlNode> implements AstBuilderMixin {
  ...
  public SqlNode visitSelect_ops_no_parens(PGParser.Select_ops_no_parensContext ctx) {
    if (ctx.select_ops() != null) {
      final SqlNode node = mkNode(SetOp);
      node.$(SetOp_Left, wrapAsQuery(ctx.select_ops().accept(this)));
      node.$(
          SetOp_Right,
          wrapAsQuery(
  ...
  private void addConstraint(SqlNode node, PGParser.Constr_bodyContext ctx) {
    ...
    else if (ctx.NOT() != null) node.flag(ColDef_Cons, ConstraintKind.NOT_NULL);
