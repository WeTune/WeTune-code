...
class NotNullInference {
  ...
  boolean isNotNullAt(Value toCheck, int surfaceId) {
    ...
    if (kind == Input) return onInput(toCheck);
    else if (kind == Join) return onJoin(toCheck, surfaceId);
    else if (kind == Filter) return onFilter(toCheck, surfaceId);
    else if (kind == Proj) return onProj(toCheck, surfaceId);
    else if (kind == Agg) return onAgg(toCheck, surfaceId);
