...
class PlanEq {
  ...
  PlanEq(PlanContext plan0, PlanContext plan1) {
    this.plan0 = plan0;
    this.plan1 = plan1;
  }
  ...
  private boolean isEqJoin(int node0, int node1) {
    if (joinKindOf(plan0, node0) != joinKindOf(plan1, node1)) return false;
  ...
  private boolean isEqProj(int node0, int node1) {
    if (isDedup(plan0, node0) != isDedup(plan1, node1)) return false;
  ...
  private static List<Value> getRefBindingContext(PlanContext plan, int nodeId) {
    ...
    return join(lookup0, getRefBindingForeignLookup(plan, nodeId));
  }
