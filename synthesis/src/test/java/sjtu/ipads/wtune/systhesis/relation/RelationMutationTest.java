...
class RelationMutationTest {
  ...
  void solidus210() {
    doTest(
        ...
        "SELECT COUNT(*) FROM (SELECT 1 AS `one` FROM `spree_stock_items` INNER JOIN `spree_stock_items` AS `spree_stock_items_exposed_2_1` ON `spree_stock_items`.`id` = `spree_stock_items_exposed_2_1`.`id` LEFT JOIN `spree_stock_locations` AS `spree_stock_locations_exposed_2_1` ON `spree_stock_locations_exposed_2_1`.`id` = `spree_stock_items_exposed_2_1`.`stock_location_id` WHERE `spree_stock_items`.`deleted_at` IS NULL AND `spree_stock_items`.`stock_location_id` = 23 AND `spree_stock_locations_exposed_2_1`.`active` = TRUE LIMIT 25 OFFSET 0) AS `subquery_for_count`");
  }
