...
class ExposeDerivedTableSourceTest {
  ...
  void test() {
    ...
    stmt.setAppName("test");
    {
      ...
      assertTrue(ExposeDerivedTableSource.canExpose(target));
      ...
      assertEquals(
          "SELECT 1 FROM `a` "
              + "LEFT JOIN `b` AS `b_exposed_0` ON `a`.`i` = `b_exposed_0`.`x` "
              + "INNER JOIN `a` AS `a_exposed_0` ON `b_exposed_0`.`z` = `a_exposed_0`.`k` "
