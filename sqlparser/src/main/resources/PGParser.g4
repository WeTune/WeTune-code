...
predefined_type
    : type=BIGINT
    | type=BIT VARYING? type_length?
    | type=BOOLEAN
    | type=DEC precision_param?
    | type=DECIMAL precision_param?
    | type=DOUBLE PRECISION
    | type=FLOAT precision_param?
    | type=INT
    | type=INTEGER
    | type=INTERVAL interval_field? type_length?
    | NATIONAL? type=(CHARACTER | CHAR) VARYING? type_length?
    | type=NCHAR VARYING? type_length?
    | type=NUMERIC precision_param?
    | type=REAL
    | type=SMALLINT
    | type=TIME type_length? ((WITH | WITHOUT) TIME ZONE)?
    | type=TIMESTAMP type_length? ((WITH | WITHOUT) TIME ZONE)?
