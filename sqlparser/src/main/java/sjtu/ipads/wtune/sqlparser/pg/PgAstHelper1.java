...
import static sjtu.ipads.wtune.sqlparser.ast1.constants.IndexKind.*;
import static sjtu.ipads.wtune.sqlparser.ast1.constants.JoinKind.*;

public interface PgAstHelper1 {
  ...
  static JoinKind parseJoinKind(PGParser.From_itemContext ctx) {
    if (ctx == null) return null;
    if (ctx.NATURAL() != null) {
      if (ctx.LEFT() != null) return NATURAL_LEFT_JOIN;
      else if (ctx.RIGHT() != null) return NATURAL_RIGHT_JOIN;
      else return NATURAL_INNER_JOIN;
    }

    if (ctx.CROSS() != null) return CROSS_JOIN;
    if (ctx.INNER() != null) return INNER_JOIN;
    if (ctx.LEFT() != null) return LEFT_JOIN;
    if (ctx.RIGHT() != null) return RIGHT_JOIN;
    if (ctx.FULL() != null) return FULL_JOIN;
