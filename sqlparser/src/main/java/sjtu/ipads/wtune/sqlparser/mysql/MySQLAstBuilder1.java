...
import static sjtu.ipads.wtune.sqlparser.ast1.constants.IndexKind.FULLTEXT;
import static sjtu.ipads.wtune.sqlparser.ast1.constants.IndexKind.SPATIAL;
import static sjtu.ipads.wtune.sqlparser.ast1.constants.UnaryOpKind.BINARY;
import static sjtu.ipads.wtune.sqlparser.ast1.constants.UnaryOpKind.NOT;
import static sjtu.ipads.wtune.sqlparser.mysql.MySQLAstHelper1.*;

class MySQLAstBuilder1 extends MySQLParserBaseVisitor<SqlNode> implements AstBuilderMixin {
  ...
  public SqlNode visitQuerySpecification(MySQLParser.QuerySpecificationContext ctx) {
    ...
    if (windowClause != null)
      node.$(
          QuerySpec_Windows,
          mkNodes(listMap(windowClause.windowDefinition(), this::visitWindowDefinition)));
  ...
  public SqlNode visitSingleTable(MySQLParser.SingleTableContext ctx) {
    ...
    if (ctx.usePartition() != null) {
      ...
      node.$(Simple_Partition, listMap(identifiers, MySQLASTHelper::stringifyIdentifier));
    }
    ...
    if (ctx.indexHintList() != null)
      node.$(Simple_Hints, mkNodes(listMap(ctx.indexHintList().indexHint(), this::visitIndexHint)));
  ...
  public SqlNode visitJoinedTable(MySQLParser.JoinedTableContext ctx) {
    ...
    if (ctx.identifierListWithParentheses() != null) {
      ...
      node.$(Joined_Using, listMap(identifiers, MySQLASTHelper::stringifyIdentifier));
    }
  ...
  public SqlNode visitSimpleExprConcat(MySQLParser.SimpleExprConcatContext ctx) {
    ...
    node.$(FuncCall_Args, mkNodes(listMap(ctx.simpleExpr(), arg -> arg.accept(this))));
    return node;
  }
