...
import static sjtu.ipads.wtune.sqlparser.ast1.ExprKind.Array;
...
import static sjtu.ipads.wtune.sqlparser.ast1.TableSourceKind.JoinedSource;
import static sjtu.ipads.wtune.sqlparser.ast1.constants.BinaryOpKind.MEMBER_OF;
...
public class SqlFormatter implements SqlVisitor{
  ...
  public boolean enterVariable(SqlNode variable) {
    append(variable.$(Variable_Scope).prefix());
    append(variable.$(Variable_Name));

    final SqlNode assignment = variable.$(Variable_Assignment);
    if (assignment != null) {
      append(" = ");
  ...
  public boolean enterLiteral(SqlNode literal) {
    final Object value = literal.$(Literal_Value);
  ...
  public boolean enterFuncCall(SqlNode funcCall) {
    final SqlNode name = funcCall.$(FuncCall_Name);
    final SqlNodes args = coalesce(funcCall.$(FuncCall_Args), SqlNodes.mkEmpty());
  ...
  public boolean enterCollation(SqlNode collation) {
    final SqlNode expr = collation.$(Collate_Expr);
    try (final var ignored = withParen(needParen(collation, expr, true))) {
      safeVisit(expr);
    }
    append(" COLLATE ");
    if (PostgreSQL.equals(collation.dbType())) {
      collation.$(Collate_Collation).accept(this);
    } else {
      append('\'').append(collation.$(Collate_Collation).$(Symbol_Text)).append('\'');
    }
    return false;
  }
  ...
  public boolean enterParamMarker(SqlNode paramMarker) {
    if (PostgreSQL.equals(paramMarker.dbType())
        && !paramMarker.isFlag(Param_ForceQuestion)) {
      append('$').append(coalesce(paramMarker.$(Param_Number), 1));
    } else {
      append('?');
    }
    return false;
  ...
  public boolean enterUnary(SqlNode unary) {
    final String op = unary.$(Unary_Op).text();
    append(op);
    if (op.length() > 1) append(' ');

    final SqlNode expr = unary.$(Unary_Expr);
  ...
  public boolean enterGroupingOp(SqlNode groupingOp) {
    append("GROUPING");
    appendNodes(coalesce(groupingOp.$(GroupingOp_Exprs), SqlNodes.mkEmpty()));
    return false;
  }

  @Override
  public boolean enterTuple(SqlNode tuple) {
    if (tuple.isFlag(Tuple_AsRow)) append("ROW");
    appendNodes(coalesce(tuple.$(Tuple_Exprs), SqlNodes.mkEmpty()), true, true);
    return false;
  }
  ...
  public boolean enterMatch(SqlNode match) {
    append("MATCH ");
    appendNodes(match.$(Match_Cols), false);

    append(" AGAINST (");
    safeVisit(match.$(Match_Expr));

    final MatchOption option = match.$(Match_Option);
  ...
  public boolean enterCast(SqlNode cast) {
    if (PostgreSQL.equals(cast.dbType())) {
      safeVisit(cast.$(Cast_Expr));
      append("::");
      cast.$(Cast_Type).formatAsCastType(builder, PostgreSQL);
  ...
  public boolean enterDefault(SqlNode _default) {
    append("DEFAULT(");
    safeVisit(_default.$(Default_Col));
    append(')');
    return false;
  }
  ...
  public boolean enterSymbol(SqlNode symbol) {
    append(symbol.$(Symbol_Text).toUpperCase());
    return false;
  }
  ...
  public boolean enterInterval(SqlNode interval) {
    append("INTERVAL ");
    final SqlNode expr = interval.$(Interval_Expr);
    try (final var ignored = withParen(needParen(interval, expr, false))) {
      safeVisit(expr);
    }
    append(' ').append(interval.$(Interval_Unit));
  ...
  public boolean enterWildcard(SqlNode wildcard) {
    final SqlNode table = wildcard.$(Wildcard_Table);
    if (table != null) {
      safeVisit(table);
      append('.');
    }
  ...
  public boolean enterAggregate(SqlNode aggregate) {
    append(aggregate.$(Aggregate_Name).toUpperCase()).append('(');
    if (aggregate.isFlag(Aggregate_Distinct)) append("DISTINCT ");

    appendNodes(aggregate.$(Aggregate_Args), false, true);
    final SqlNodes order = aggregate.$(Aggregate_Order);
    ...
    final String sep = aggregate.$(Aggregate_Sep);
    if (sep != null) append(" SEPARATOR '").append(sep).append('\'');

    append(')');

    final String windowName = aggregate.$(Aggregate_WindowName);
    final SqlNode windowSpec = aggregate.$(Aggregate_WindowSpec);
    if (windowName != null) {
      append(" OVER ");
  ...
  public boolean enterExists(SqlNode exists) {
    append("EXISTS ");
    safeVisit(exists.$(Exists_Subquery));
    return false;
  }
  ...
  public boolean enterJoinedTableSource(SqlNode joinedTableSource) {
    ...
    final boolean needParen = JoinedSource.isInstance(right);
  ...
  public boolean enterConvertUsing(SqlNode convertUsing) {
    append("CONVERT(");
    safeVisit(convertUsing.$(ConvertUsing_Expr));
    append(" USING ");

    String charset = convertUsing.$(ConvertUsing_Charset).$(Symbol_Text);
  ...
  public boolean enterCase(SqlNode _case) {
    append("CASE");
    final SqlNode cond = _case.$(Case_Cond);
    if (cond != null) {
      append(' ');
  ...
  public boolean enterWhen(SqlNode when) {
    append("WHEN ");
    final SqlNode cond = when.$(When_Cond);
    try (final var ignored = withParen(needParen(when, cond, false))) {
      safeVisit(cond);
    }

    append(" THEN ");
    final SqlNode expr = when.$(When_Expr);
  ...
  public boolean enterBinary(SqlNode binary) {
    final SqlNode left = binary.$(Binary_Left);
    try (final var ignored = withParen(needParen(binary, left, true))) {
      safeVisit(left);
    }

    final BinaryOpKind op = binary.$(Binary_Op);
    ...
    final SubqueryOption subqueryOption = binary.$(Binary_SubqueryOption);
    if (subqueryOption != null) append(subqueryOption).append(' ');

    final SqlNode right = binary.$(Binary_Right);

    final boolean needParen =
        op == MEMBER_OF || Array.isInstance(right) || needParen(binary, right, false);
  ...
  public boolean enterTernary(SqlNode ternary) {
    final TernaryOp operator = ternary.$(Ternary_Op);
    final SqlNode left = ternary.$(Ternary_Left);
    final SqlNode middle = ternary.$(Ternary_Middle);
    final SqlNode right = ternary.$(Ternary_Right);
  ...
  public boolean enterQueryExpr(SqlNode queryExpr) {
    try (final var ignored = withParen(true)) {
      increaseIndent();
      breakLine(false);
      safeVisit(queryExpr.$(QueryExpr_Query));
      decreaseIndent();
      breakLine(false);
    }
    return false;
  ...
  public boolean enterArray(SqlNode array) {
    append("ARRAY[");
    appendNodes(array.$(Array_Elements), false, true);
    append(']');
    return false;
  }
