...
interface MySQLAstHelper1 {
  ...
  static String stringifyText(MySQLParser.TextLiteralContext text) {
    return String.join("", ListSupport.<MySQLParser.TextStringLiteralContext, String>map((Iterable<MySQLParser.TextStringLiteralContext>) text.textStringLiteral(), (Function<? super MySQLParser.TextStringLiteralContext, ? extends String>) MySQLAstHelper1::stringifyText));
  }
  ...
  static void collectColumnAttr(MySQLParser.ColumnAttributeContext attrs, SqlNode out) {
    if (attrs.NOT_SYMBOL() != null && attrs.nullLiteral() != null)
      out.flag(ColDef_Cons, NOT_NULL);
    if (attrs.UNIQUE_SYMBOL() != null) out.flag(ColDef_Cons, UNIQUE);
    if (attrs.PRIMARY_SYMBOL() != null) out.flag(ColDef_Cons, PRIMARY);
    if (attrs.checkConstraint() != null) out.flag(ColDef_Cons, ConstraintKind.CHECK);
    if (attrs.DEFAULT_SYMBOL() != null) out.flag(ColDef_Default);
    if (attrs.AUTO_INCREMENT_SYMBOL() != null) out.flag(ColDef_AutoInc);
  }

  static void collectGColumnAttr(MySQLParser.GcolAttributeContext attrs, SqlNode out) {
    if (attrs.notRule() != null) out.flag(ColDef_Cons, NOT_NULL);
    if (attrs.UNIQUE_SYMBOL() != null) out.flag(ColDef_Cons, UNIQUE);
    if (attrs.PRIMARY_SYMBOL() != null) out.flag(ColDef_Cons, PRIMARY);
  }
