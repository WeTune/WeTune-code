...
public class FilterJoinQuery extends BaseQueryBuilder {
  @Override
  protected Function<Value, Proposition> semantic() {
    ...
    p0.setViableSources(powerSet(Set.of(t0, t1)));
    ...
    final Proposition join = p1.apply(a).equalsTo(p2.apply(p3.apply(b)));
    final Proposition filter = (Proposition) pred.apply(p0.apply(a, p3.apply(b)));

    return x ->
        ctx()
            .makeExists(
                asArray(a, b),
                x.equalsTo(ctx().makeCombine(a, p3.apply(b))).and(from).and(filter).and(join));
  }
}
