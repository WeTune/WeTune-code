...
import sjtu.ipads.wtune.common.utils.IntHistogram;
import sjtu.ipads.wtune.common.utils.IntStatistic;
...
import static sjtu.ipads.wtune.common.utils.IterableSupport.count;
...
public class Statistics {
  ...
  private int calcNumConstraintsNoPrune(Fragment src, Fragment dest) {
    ...
    int numTables = syms0.symbolsOf(TABLE).size() + syms1.symbolsOf(TABLE).size();
    int numAttrs = syms0.symbolsOf(ATTRS).size() + syms1.symbolsOf(ATTRS).size();
    int numPreds = syms0.symbolsOf(PRED).size() + syms1.symbolsOf(PRED).size();
