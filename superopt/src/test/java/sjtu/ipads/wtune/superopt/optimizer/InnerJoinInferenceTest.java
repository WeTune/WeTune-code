...
public class InnerJoinInferenceTest {
  ...
  void test1() {
    final PlanContext plan =
        parsePlan("Select a.i From a Left Join b On a.i=b.x Left Join c On c.u=b.y Where c.v=0");
    new InnerJoinInference(plan).inferenceAndEnforce(plan.root());
    assertEquals(
  ...
  void test2() {
    final PlanContext plan =
        parsePlan("Select a.i From a Left Join b On a.i=b.x Left Join c On c.u=b.y Where b.y=0");
    new InnerJoinInference(plan).inferenceAndEnforce(plan.root());
    assertEquals(
