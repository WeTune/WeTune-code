...
class FingerprintImpl implements Fingerprint {
  ...
  FingerprintImpl(String fingerprint) {
  ...
  public static FingerprintImpl mk(Fragment fragment) {
    final OpFingerprintBuilder builder = new OpFingerprintBuilder();
    return builder.build(fragment.root(), 4);
  }

  public static Set<FingerprintImpl> mk(PlanContext plan, int node) {
    final Set<FingerprintImpl> fingerprints = new HashSet<>();
  ...
  private static char getOpIdentifier(OperatorType kind, boolean dedup) {
    switch (kind) {
      case PROJ:
        return dedup ? 'q' : 'p';
      case SIMPLE_FILTER:
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof FingerprintImpl)) return false;
    final FingerprintImpl that = (FingerprintImpl) o;
  ...
  private static class OpFingerprintBuilder {
    ...
    private FingerprintImpl fingerprint;

    private FingerprintImpl build(Op op, int limit) {
      if (limit <= 0 || op.kind() == INPUT) {
        return fingerprint = new FingerprintImpl(builder.toString());
      }
  ...
  private static class PlanFingerprintBuilder {
    ...
    private final Set<FingerprintImpl> fingerprints;
    ...
    private Set<FingerprintImpl> build(int node, int budget) {
      final PlanKind nodeKind = plan.kindOf(node);
      if (budget <= 0 || nodeKind == PlanKind.Input) {
        fingerprints.add(new FingerprintImpl(builder.toString()));
        return fingerprints;
      }
