...
class PlanTranslator2 {
  ...
  Pair<PlanContext, PlanContext> translate() {
    assignAll();
    schema = mkSchema();
    final PlanContext sourcePlan = new PlanTranslator(rule._0(), false).translate();
    final PlanContext targetPlan = new PlanTranslator(rule._1(), true).translate();
  ...
  private class PlanTranslator {
    ...
    private PlanTranslator(Fragment template, boolean isTargetSide) {
