...
public abstract class LabeledTreeNodesBase<
  ...
  protected LabeledTreeNodesBase(C context, TIntList nodeIds) {
    ...
    this.nodeIds = coalesce(nodeIds, new TIntArrayList(0));
  ...
  public boolean erase(int nodeId) {
    if (nodeIds.remove(nodeId)) {
      ...
      if (parentId != NO_SUCH_NODE && context.parentOf(nodeId) != NO_SUCH_NODE)
  ...
  public boolean add(N n) {
    ...
    if (parentId != NO_SUCH_NODE) context.setParentOf(n.nodeId(), parentId);
    return true;
  }

  @Override
  public int set(int index, int newNodeId) {
    checkNodePresent(context, newNodeId);
  ...
  public boolean remove(Object o) {
    ...
    if (nodeIds.remove(nodeId)) {
      if (parentId != NO_SUCH_NODE) context.detachNode(nodeId);
      return true;
    } else {
      return false;
    }
